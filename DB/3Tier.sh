...
echo "test in then"
vm_ip=$(cat $OVF_ENV | grep DB_IP  | cut -d '"' -f 4 | head -1)
vm_subnet=$(cat $OVF_ENV | grep DB_Subnet  | cut -d '"' -f 4 | head -1)
vm_gateway=$(cat $OVF_ENV | grep DB_Gateway  | cut -d '"' -f 4 | head -1)
vm_hostname=$(cat $OVF_ENV | grep DB_IP  | cut -d '"' -f 2 | cut -d "_" -f 1  | head -1)
